...
if UNSUPERVISED:
    name_prefix = f"unsupervised_{model_name}"
else:
    name_prefix = f"supervised_{model_name}_with_{training_data_name}"
...
model.save(log_dir + f"{string_timesteps}_{model_name}")

# env.set_record_option("test_bk2s/.")

# Load the trained agent to check it's saved properly
model = model_class.load(log_dir + f"{string_timesteps}_{model_name}")
...
animation_path += f"_{model_name}_{string_timesteps}.gif"
